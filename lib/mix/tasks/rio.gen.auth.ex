defmodule Mix.Tasks.Rio.Gen.Auth do
  ...
  def run(args, test_opts \\ []) do
    ...
    binding = [
      ...
      web_module_prefix: Module.concat(schema.web_namespace, schema.alias),
  ...
  defp files_to_be_generated(%Context{schema: schema, context_app: context_app} = context) do
    ...
    [
      ...
      {:eex, "registration_live.html.heex",
       Path.join([
         web_prefix,
         "modules",
         web_path,
         "#{schema.singular}_registration",
         "#{schema.singular}_registration_live.html.heex"
       ])},
      {:eex, "registration_live.ex",
       Path.join([
         ...
         web_path,
         "#{schema.singular}_registration",
         "#{schema.singular}_registration_live.ex"
       ])},
      ...
      {:eex, "reset_password_live.ex",
       Path.join([
         ...
         "#{schema.singular}_reset_password_live.ex"
       ])},
      ...
      {:eex, "session_live.html.heex",
       Path.join([
         web_prefix,
         "modules",
         web_path,
         "#{schema.singular}_session",
         "#{schema.singular}_session_live.html.heex"
       ])},
      {:eex, "session_live.ex",
       Path.join([web_prefix, "modules", web_path, "#{schema.singular}_session", "#{schema.singular}_session_live.ex"])}
